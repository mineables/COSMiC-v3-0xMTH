const INFURA_MAINNET_URL = 'https://dai.poa.network';
const MINING_POOL_URL = '';
...
var quarryJSON = require('../contracts/MithrilTokenQuarry')
...
module.exports = {
    ...
    async init(web3, miningLogger) {
        ...
        if (!this.encryptedStorageExists()) {
            console.log('Storage doesnt exist, creating dir..')
            //build the storage folders if they (probably) dont exist anymore
	    let results = await mkDirpAsync(this.get0xBitcoinLocalFolderPath())
	    console.log(results)
	    /*
            await mkdirp(this.get0xBitcoinLocalFolderPath(), (err) => {
	    ...
	    */
	    ...
	    console.log('Storage exists! Good to go...')
	}

        localStorage = new LocalStorage(this.get0xBitcoinLocalFolderPath() + '/0xmithrilminerstore');

        let addresses = await addr.addresses
	
	console.log(addresses)

        if (!this.vaultDataExists(localStorage)) {
            localStorage.setItem('vaultdata', JSON.stringify({ vault: { data: this.getInitialVaultDataJson(addresses), version: VAULT_DATA_VERSION } }))
	    ...
	    this.quarryContract = new web3.eth.Contract( quarryJSON.abi, addresses.QUARRY )
    ...
    get0xBitcoinLocalFolderPath() {
        return this.getOSLocalDataFolderPath() + '/.0xmithril';
    },
    ...
    async handleAccountCommand(subsystem_command, subsystem_option) {
        ...
        if (subsystem_command === 'list') {
            console.log('.0xMithril Ethereum accounts list:');
	...
	if (subsystem_command === 'import') {
            this.importAccount(subsystem_option);
	...
	if (subsystem_command === 'remove') {
            this.removeAccount(subsystem_option);
            await this.saveVaultData();
        }
    ...
    async handleContractCommand(subsystem_command, subsystem_option) {
        if (subsystem_command === 'select') {
            ...
            await this.saveVaultData();
	    this.tokenContract = new this.web3.eth.Contract(tokenContractJSON.abi, this.getTokenContractAddress())
    ...
    async handleConfigCommand(subsystem_command, subsystem_option) {
        if (subsystem_command === 'vardiff') {
            this.setVardiff(subsystem_option);
            await this.saveVaultData();
        }
        ...
        if (subsystem_command === 'artgasprice') {
            this.setArtifactOpsGasPrice(subsystem_option);
            await this.saveVaultData();
        }
        ...
        if (subsystem_command === 'list') {
	        ...
	        console.log('Vardif:', this.getVardiff())
            console.log('CPU Threads:', this.getNumThreads())
            console.log('Gas Price (gwei):', this.getGasPriceGwei())
            console.log('Artifact Ops Gas Price (gwei):', this.getArtifactOpsGasPriceGwei());
        }
    },
	...
	vaultData.account_list.splice(index, 1)
    },

    async importAccount(private_key) {
	/*
	account = {
           address: "",
           privateKey: ""
    ...
    setVardiff(vardiff) {
	vaultData.vardiff = vardiff
        miningLogger.print('Set custom vardiff: ', vardiff)
    },

    getVardiff() {
        return vaultData.vardiff;
    },
	...
	this.web3.setProvider(url)
        this.tokenContract = new this.web3.eth.Contract(tokenContractJSON.abi, this.getTokenContractAddress())
	this.quarryContract = new this.web3.eth.Contract( quarryJSON.abi, addr.QUARRY )
    ...
    encryptedStorageExists() {
        if (!fs.existsSync(this.get0xBitcoinLocalFolderPath()) || !fs.existsSync(this.get0xBitcoinLocalFolderPath() + '/0xmithrilminerstore')) {
            return false;
        }

        if (!fs.existsSync(this.get0xBitcoinLocalFolderPath() + '/0xmithrilminerstore/vaultdata')) {
            return false;
        }
    ...
    setArtifactOpsGasPrice(gprice) // in gwei
    {
        var f_gprice = parseFloat(gprice);

        if (f_gprice > 0.0001 && f_gprice < 100000) {
            vaultData.artifact_ops_gas_price = f_gprice;
            miningLogger.print('Set artifact ops gas price (gwei):', f_gprice);
        }
    ...
    getArtifactOpsGasPriceGwei() {
        return vaultData.artifact_ops_gas_price;
    },

    getInitialVaultDataJson(addresses) {
        var datastore = {
	    ...
	    "vardiff": 65536,
            ...
            "selected_contract_address": addresses.MITHRIL,
            "artifact_ops_gas_price": 1
        }
